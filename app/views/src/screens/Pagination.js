  ...
  return (
    <div className="row justify-content-center">
      <nav aria-label="Playlists search results page.">
        ...
        {page === 1 && // first page and previous button disabled
          <ul className="pagination">
            <li className="page-item disabled">
              <Link to="" tabIndex="-1" className="page-link">Previous</Link>
            </li>
            ...
            {playlists.length > resultsPerPage &&
              <li className="page-item">
                <Link className="page-link" to={`${match.url}?page=${page + 1}`}>{page + 1}</Link>
              </li>
            }

            {playlists.length > resultsPerPage &&
              <li className="page-item">
                <Link className="page-link" to={`${match.url}?page=${page + 1}`}>Next</Link>
              </li>
            }
          </ul>
        }

        {page > 1 && playlists.length > resultsPerPage && // previous button + all the other buttons
          <ul className="pagination">
            <li className="page-item">
              <Link className="page-link" to={`${match.url}?page=${page - 1}`}>Previous</Link>
            </li>

            <li className="page-item">
              <Link to={`${match.url}?page=${page - 1}`} className="page-link">{page - 1}</Link>
            </li>
        ...
        {playlists.length < resultsPerPage && page !== 1 && // we are on the last page
          <ul className="pagination">
            <li className="page-item">
              <Link className="page-link" to={`${match.url}?page=${page - 1}`}>Previous</Link>
            </li>
